...
class TiledMap:
    ...
    def find_named_spawn(self, spawn_name):
        """포탈 target_spawn 이름으로 스폰 찾기.
        못 찾으면 find_player_spawn()으로 대체."""
        if not spawn_name:
...
def main_menu():
    ...
    desc_btn = Button(WIDTH // 2 - 200, 750, 400, 80, "설명", ORANGE, (255, 180, 80))
    back_btn = Button(50, 50, 140, 60, "뒤로가기", DARK_GRAY, LIGHT_GRAY)
    ...
    woman_btn = Button(WIDTH // 2 + 120, 640, 240, 70, "여자 선택", (255, 105, 180), (255, 150, 200))
    man_btn = Button(WIDTH // 2 - 360, 640, 240, 70, "남자 선택", BLUE, (0, 150, 255))

    menu_state = "main" 
    instructions = [
        "이동: WASD",
        "마법 시전: 마우스 좌클릭 / 스페이스 (타겟 필요)",
        "마법 슬롯 선택: 1~4",
        "마법/타겟 상태 보기: E (마법 선택 창)",
        "마법 슬롯 창: E + M (마법 장착 창)",
        "무기 선택: E + N (무기 선택 창)",
        "인벤토리: I",
        "자동 타겟 토글: V",
    ...
    while True:
        ...
        if menu_state == "main":
            ...
            title = get_korean_font(48).render("게임 설명", True, WHITE)
...
def draw_skill_menu(surface, player, target_set, selected_slot):
    ...
    small = small_font.render("무기 선택: M (무기 창 열기)", True, LIGHT_GRAY)
...
def draw_weapon_menu(surface, player, available_weapons, selected_index):
    ...
    list_title = item_font.render("보유 무기 목록", True, WHITE)
            ...
            for enemy in current_map.enemies[:]:
                ...
                if isinstance(enemy, Slime):
                    if enemy.death_finished:
                        ...
                        if 5 <= level <= 15:
                            staff_candidates.append("초급 지팡이")
                        if 10 <= level <= 20:
                            staff_candidates.append("중급 지팡이")
                        if 20 <= level <= 30:
                            staff_candidates.append("상급 지팡이")
                        if 25 <= level <= 40:
